...
void Ray_Trace_Adaptively(long my_node) {
  ...
  while (Global_Queue[num_nodes][0] > 0) {
    ...
    work = Global_Queue[local_node][0];
    Global_Queue[local_node][0] += 1;
...
void Ray_Trace_Adaptive_Box(long outx, long outy, long boxlen) {
  ...
  for (i = 0; i <= boxlen && outy + i < image_len[Y]; i += boxlen) {
    for (j = 0; j <= boxlen && outx + j < image_len[X]; j += boxlen) {
      ...
      /*reschedule processes here if rescheduling only at synch points on
       * simulator*/
  ...
  if (boxlen > lowest_volume_boxlen &&
      max_volume_color - min_volume_color >= volume_color_difference) {
...
void Ray_Trace_Fast_Non_Adaptively(long my_node) {
  ...
  for (i = 0; i < num_blocks; i += num_nodes) {
    ...
    for (outy = yindex; outy < yindex + block_ylen && outy < image_len[Y];
         outy += lowest_volume_boxlen) {
      for (outx = xindex; outx < xindex + block_xlen && outx < image_len[X];
