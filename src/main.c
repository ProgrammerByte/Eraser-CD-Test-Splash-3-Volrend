...
int main(int argc, char *argv[]) {
  ...
  /*  if (num_nodes > 1)
      {int aantal=num_nodes-1; while (aantal--) pthread_join(__tid__[aantal],
     NULL);};*/
  if (num_nodes > 1) {
    int aantal = num_nodes;
    while (aantal--)
...
void Frame() {
  ...
  { pthread_mutex_init(&(Global_CountLock), NULL); };
  {
    ...
    for (i = 0; i < (MAX_NUMPROC + 1); i++)
      pthread_mutex_init(&((Global_QLock)[i]), NULL);
  };
...

  {
    ...
    for (i = 0; i < (num_nodes)-1; i++) {
      Error = pthread_create(&__tid__[__threads__++], NULL, Render_Loop, NULL);
      if (Error != 0) {
    ...
    for (step = 0; step < ROTATE_STEPS; step++) { /* do rotation sequence */
      ...
      */
      ...
      if (my_node == ROOT) {
        if (ROTATE_STEPS > 1) {
#ifdef DIM
          sprintf(outfile, "%s_%ld", filename,
                  1000 + dim * ROTATE_STEPS + step);
#else
        sprintf(outfile, "%s_%ld.tiff", filename, 1000 + step);
          ...
          WriteGrayscaleTIFF(outfile, image_len[X], image_len[Y], image_len[X],
                             image_address);
        } else {
          ...
          WriteGrayscaleTIFF(filename, image_len[X], image_len[Y], image_len[X],
                             image_address);
        }
      }
    }
...
void Store_Image(char filename[]) {
  ...
  Write_Shorts(fd, (unsigned char *)local_image_len,
               (long)sizeof(local_image_len));
