  ...
  for (outz = zstart; outz < zstop; outz++) {
    for (outy = ystart; outy < ystop; outy++) {
      for (outx = xstart; outx < xstop; outx++) {
        ...
        grd_x = (double)((long)MAP(inz, iny, inx + 1) -
                         (long)MAP(inz, iny, inx - 1));
        grd_y = (double)((long)MAP(inz, iny + 1, inx) -
                         (long)MAP(inz, iny - 1, inx));
        grd_z = (double)((long)MAP(inz + 1, iny, inx) -
...
void Load_Normal(char filename[]) {
  ...
  Read_Shorts(fd, (unsigned char *)norm_address,
              (long)(norm_length * sizeof(NORMAL)));
  fflush(stdout);
  Close_File(fd);
}
...
void Store_Normal(char filename[]) {
  ...
  Write_Shorts(fd, (unsigned char *)norm_address,
               (long)(norm_length * sizeof(NORMAL)));
  Close_File(fd);
}
