...
#define WRITE_PYR(IBIT, ILEVEL, IZ, IY, IX)                                    \
  (PYR_ADDRESS(ILEVEL, IZ, IY, IX), *pyr_address2 |= (IBIT) << pyr_offset2)
...
void Compute_Octree() {
  ...
  */
  ...
  for (level = 1; level < pyr_levels; level++) {
    ...
    pyr_length[level] =
        (pyr_len[level][X] * pyr_len[level][Y] * pyr_len[level][Z] + 7) / 8;
...
void Load_Octree(char filename[]) {
  ...
  Read_Shorts(fd, (unsigned char *)pyr_len,
              (long)(pyr_levels * NM * sizeof(long)));
  Read_Shorts(fd, (unsigned char *)pyr_voxlen,
              (long)(pyr_levels * NM * sizeof(long)));
  Read_Longs(fd, (unsigned char *)pyr_length,
             (long)(pyr_levels * sizeof(pyr_length[0])));

  printf("    Loading binary pyramid of %d levels...\n", pyr_levels);
  for (level = 0; level < pyr_levels; level++) {
    Allocate_Pyramid_Level(&pyr_address[level], pyr_length[level]);
    printf("      Loading pyramid level %ld from .pyr file...\n", level);
    Read_Bytes(fd, (unsigned char *)pyr_address[level],
               (long)(pyr_length[level] * sizeof(BYTE)));
  }
  Close_File(fd);
}
...
void Store_Octree(char filename[]) {
  ...
  Write_Shorts(fd, (unsigned char *)pyr_len,
               (long)(pyr_levels * NM * sizeof(long)));
  Write_Shorts(fd, (unsigned char *)pyr_voxlen,
               (long)(pyr_levels * NM * sizeof(long)));
  Write_Longs(fd, (unsigned char *)pyr_length,
              (long)(pyr_levels * sizeof(pyr_length[0])));
  ...
  for (level = 0; level < pyr_levels; level++) {
    ...
    Write_Bytes(fd, (unsigned char *)pyr_address[level],
                (long)(pyr_length[level] * sizeof(BYTE)));
  }
  Close_File(fd);
}
