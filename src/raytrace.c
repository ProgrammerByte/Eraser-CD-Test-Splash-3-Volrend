...
#define VXEL(X, SIGN) ((SIGN) > 0 ? (long)((X) + SMALL) : (long)((X)-SMALL))
...
void Trace_Ray(float foutx, float fouty, PIXEL *pixel_address) {
  ...
  for (i = 0; i < NM; i++) {
    ray[0][i] = out_invvertex[0][0][0][i] + invjacobian[X][i] * foutx +
  ...
  while (1) {
    ...
    bit =
        PYR(level, ivoxel[Z] >> level, ivoxel[Y] >> level, ivoxel[X] >> level);
    ...
    if (invjacobian[Z][X] > SMALL) {
      jump[X] = invinvjacobian[Z][X] *
                (((ROUNDDOWN(voxel[X]) >> level) + 1) * pyr_voxlen[level][X] -
                 voxel[X]);
      min_jump = MIN(min_jump, jump[X]);
    } else if (invjacobian[Z][X] < -SMALL) {
      jump[X] =
          invinvjacobian[Z][X] *
          ((STEPDOWN(voxel[X]) >> level) * pyr_voxlen[level][X] - voxel[X]);
      min_jump = MIN(min_jump, jump[X]);
    }
    if (invjacobian[Z][Y] > SMALL) {
      jump[Y] = invinvjacobian[Z][Y] *
                (((ROUNDDOWN(voxel[Y]) >> level) + 1) * pyr_voxlen[level][Y] -
                 voxel[Y]);
      min_jump = MIN(min_jump, jump[Y]);
    } else if (invjacobian[Z][Y] < -SMALL) {
      jump[Y] =
          invinvjacobian[Z][Y] *
          ((STEPDOWN(voxel[Y]) >> level) * pyr_voxlen[level][Y] - voxel[Y]);
      min_jump = MIN(min_jump, jump[Y]);
    }
    if (invjacobian[Z][Z] > SMALL) {
      jump[Z] = invinvjacobian[Z][Z] *
                (((ROUNDDOWN(voxel[Z]) >> level) + 1) * pyr_voxlen[level][Z] -
                 voxel[Z]);
      min_jump = MIN(min_jump, jump[Z]);
    } else if (invjacobian[Z][Z] < -SMALL) {
      jump[Z] =
          invinvjacobian[Z][Z] *
          ((STEPDOWN(voxel[Z]) >> level) * pyr_voxlen[level][Z] - voxel[Z]);
      min_jump = MIN(min_jump, jump[Z]);
    }
  ...
  next_box:
    ...
    while (level < starting_level) {
      if (next_ivoxel[X] >> (level + 1) != ivoxel[X] >> (level + 1) ||
          next_ivoxel[Y] >> (level + 1) != ivoxel[Y] >> (level + 1) ||
          next_ivoxel[Z] >> (level + 1) != ivoxel[Z] >> (level + 1)) {
        level++;
      } else
        break;
    }
  ...
  for (znorm = zstart; znorm < zstop; znorm++) {
    for (ynorm = -norm_lshift; ynorm <= norm_lshift; ynorm++) {
      ...
      if (mag > error) {
        ...
        spartial_product1 =
            normal[Z] * obshighlight[Z] + normal[Y] * obshighlight[Y];
