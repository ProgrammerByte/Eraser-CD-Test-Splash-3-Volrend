...
void Compute_Pre_View() {
  ...
  for (outz = 0; outz <= image_zlen; outz++) {
    depth_cueing[outz] =
        depth_hither - pow((float)(outz) / (float)image_zlen, depth_exponent) *
...
void Select_View(float delta_angle, long axis) {
  ...
  Inverse_Concatenate_Translation(invmatrix, (float)(out_diag_len[X]) / 2.0,
  ...
  Inverse_Concatenate_Translation(invmatrix, -out_diag_length / 2.0,
                                  -out_diag_length / 2.0,
                                  -out_diag_length / 2.0);
...
void Compute_Input_Dimensions() {
  ...
  for (z = 0; z < 2; z++) {
    for (y = 0; y < 2; y++) {
      for (x = 0; x < 2; x++) {
        Transform_Point(in_invvertex[z][y][x][X], in_invvertex[z][y][x][Y],
                        in_invvertex[z][y][x][Z], &out_invvertex[z][y][x][X],
                        &out_invvertex[z][y][x][Y], &out_invvertex[z][y][x][Z]);
      }
    }
  }
}
...
void Compute_Input_Unit_Vector() {
  ...
  for (z = 0; z < 2; z++) {
    for (y = 0; y < 2; y++) {
      for (x = 0; x < 2; x++) {
        Transform_Point(uin_invvertex[z][y][x][X], uin_invvertex[z][y][x][Y],
                        uin_invvertex[z][y][x][Z], &uout_invvertex[z][y][x][X],
...
void Transform_Point(float xold, float yold, float zold, float *xnew,
                     float *ynew, float *znew) {
  *xnew = xold * transformation_matrix[0][0] +
          yold * transformation_matrix[1][0] +
          zold * transformation_matrix[2][0] + transformation_matrix[3][0];

  *ynew = xold * transformation_matrix[0][1] +
          yold * transformation_matrix[1][1] +
          zold * transformation_matrix[2][1] + transformation_matrix[3][1];

  *znew = xold * transformation_matrix[0][2] +
          yold * transformation_matrix[1][2] +
          zold * transformation_matrix[2][2] + transformation_matrix[3][2];
}

void Inverse_Concatenate_Translation(float matrix[4][4], float xoffset,
...
void Inverse_Concatenate_Scaling(float matrix[4][4], float xscale, float yscale,
                                 float zscale) {
...
void Inverse_Concatenate_Rotation(float matrix[4][4], float axis, float angle) {
...
void Load_Translation_Matrix(float matrix[4][4], float xoffset, float yoffset,
                             float zoffset) {
...
void Load_Scaling_Matrix(float matrix[4][4], float xscale, float yscale,
                         float zscale) {
  Load_Identity_Matrix(matrix);
  matrix[0][0] = xscale;
  matrix[1][1] = yscale;
...
void Load_Rotation_Matrix(float matrix[4][4], long axis, float angle) {
  Load_Identity_Matrix(matrix);
  if (axis == XAXIS) {
...
void Concatenate_Transform(float composite_matrix[][4],
...
void Multiply_Matrices(float input_matrix1[][4], float input_matrix2[][4],
                       float output_matrix[][4]) {
  long i, j;
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) {
      output_matrix[i][j] = input_matrix1[i][0] * input_matrix2[0][j] +
                            input_matrix1[i][1] * input_matrix2[1][j] +
                            input_matrix1[i][2] * input_matrix2[2][j] +
                            input_matrix1[i][3] * input_matrix2[3][j];
    }
  }
}
